...
use crate::models::{AppEvent, DiscordApiMessage, DiscordMessage, MessageStatus};
                ...
                if let Ok((ws_stream, _)) = connect_async(&target_url).await {
                            ...
                            match msg {
                                Message::Text(txt) => {
                                    if let Ok(val) = serde_json::from_str::<serde_json::Value>(&txt) {
                                        ...
                                        } else if msg_type == "MessageResult" {
                                            let nonce = val["nonce"].as_str().unwrap_or("").to_string();
                                        ...
                                        } else if msg_type == "ChannelHistory" {
                                            if let Ok(raw_msgs) = serde_json::from_value::<Vec<DiscordApiMessage>>(val["messages"].clone()) {
                    ...
                    while let Some(job) = outbound_rx.recv().await {
                        ...
                        let payload = match job {
                            AppEvent::FetchChannelHistory(cid) => serde_json::json!({
                                "type": "FetchHistory",
                                "channel_id": cid,
                                "limit": 50
                            }),
                            AppEvent::HttpTriggerTyping => serde_json::json!({
                                "type": "SendTyping",
                                "channel_id": active_cid
                            }),
                            AppEvent::HttpSendChat { nonce, text } => serde_json::json!({
                                "type": "SendMessage",
                                "channel_id": active_cid,
                                "content": text,
                                "nonce": nonce
            ...
            while let Some(job) = outbound_rx.recv().await {
                ...
                match job {
                    ...
                    AppEvent::HttpTriggerTyping => {
                        let cid = client.token.clone(); // use client handle
                        tokio::spawn(async move {
                            let _ = client.send_typing(&cid).await;
                        });
                    }
                    AppEvent::HttpSendChat { nonce, text } => {
                        // Channel ID passed in text payload context
